...
class CreateViewSalaoHome extends Migration {
    ...
    public function up() {
       ...
       count(distinct (`cortai`.`avaliacoes`.`id`))  AS `qtd_avaliacao`,
       ...
       ifnull(max(`cortai`.`servicos`.`valor`), 0)   AS `maior_valor`
        from `cortai`.`saloes`
                 ...
                 left join `cortai`.`avaliacoes` on (`cortai`.`horarios`.`id` = `cortai`.`avaliacoes`.`horario_id` and
                                                     avaliacoes.id = horarios.avaliacao_id)
                 inner join `cortai`.`servicos` on (`cortai`.`servicos`.`salao_id` = `cortai`.`saloes`.`id`)
        where (`cortai`.`servicos`.`deleted_at` is null)
          and (`cortai`.`saloes`.`deleted_at` is null)
        group by saloes.id;");
    }
